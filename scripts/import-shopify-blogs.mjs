const SHOP_URL = "https://dalitrip.com";
...
const normalizeImageUrl = (url = "") => {
  ...
  if (decoded.startsWith("http://dalitrip.com/")) return decoded.replace("http://", "https://");
...
const getShopifyBlogFeeds = async () => {
  ...
  const handles = [...new Set(
    [...sitemap.matchAll(/<loc>(https:\/\/dalitrip\.com\/blogs\/[^/<]+)<\/loc>/g)]
...
const parseFeed = async ({ handle, url }) => {
  ...
  for (const [index, match] of entries.entries()) {
    ...
    const slug = `shopify-${handle}-${index + 1}`;
...
const run = async () => {
  ...
  console.log(JSON.stringify({ feeds: feeds.length, posts: posts.length, ...counts }, null, 2));
};
